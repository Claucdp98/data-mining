...
Escribe la fecha actual (**NOTE-01**): 4 de junio 2021
...
```{r}
NOMBRE_COMPLETO = "Claudio Galván Torres"
DIGITOS_MATRICULA = 01378814
```
...
- Densidad de la matriz (**NOTE-10**):0.56667
...
```{r}
try(
  plot(smalltx, main = paste0("Elaborado por:  ClaudioGalvan", " (", 01378814, ")" ))  
)
...
```{r}
try(
  itemFrequencyPlot(smalltx, topN=10, main = paste0("Elaborado por:ClaudioGalvan ", " (", 01378814, ")" ))
)
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):No existe una relación del producto (pañales)con otro artículo.
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**):Las metricas support y confidence incluyen a la regla. 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**):Representar cada una de las sesiones del DataSet. Su proposito es distinguir el numero de sesiones que los usuarios tienen en spotify para hacer recomendaciones al usuario.  
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Distinguir las canciones y el tipo de musica que escuchan los usuarios de spotify para poder hacer recomendaciones con respecto a lo identificado. 
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**):Si, ya que con base a estas reglas de asociación, spotify puede identificar los gustos musicales de cada usuario y hacer recomendaciones con respecto a los resultados obtenidos. 
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**):t_bacf06d3-9185-4183-84ea-ff0db51475ce y frecuencia 891  
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**):20
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios tienen 14.62 sesiones en promedio. 
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**):Las reglase de asociacion anteriores, no cumplen con con los parametros establecidos bajo los valores de support y confidence
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Debido al numero de reglas de asociacion, se puede perder la precision o exactitud en el analisis.
...
```{r}
try(
  plot(rules, main=paste0("Elaborado por:ClaudioGalvan ",  " (", 01378814, ")" ))
)
